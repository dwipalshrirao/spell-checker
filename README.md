...
Powered by a local LLM (via Ollama). Available as a Python CLI, a web app, or a native macOS menu-bar app.
...
CLI                  ──HTTP───▶  Backend (port 8000)  ────▶  Ollama (port 11434)
```
...
| CLI | Python (requests) | Quick terminal checks |
...
docs/        # Component docs + eval specs
...
- **Model**: `qwen3.5:9b-mlx` (override via `GRAMMARCHECK_OLLAMA_MODEL`)
